"""
Vulnerability analysis using an optional Anthropic Claude API call.
Falls back to a structured deterministic analysis if no API key is set.
"""
...
def analyze_fallback(cve: dict) -> dict:
    ...
    references = cve.get("reference_details", [])
    reference_tags = {tag.lower() for ref in references for tag in ref.get("tags", [])}

    network_attack = "AV:N" in vector
    no_auth = "PR:N" in vector
    no_interaction = "UI:N" in vector
    high_impact = any(token in vector for token in ("C:H", "I:H", "A:H"))
    public_exploit_signal = bool(reference_tags.intersection({"exploit", "third party advisory"}))

    # Exploitation likelihood from CVSS and public exploit/advisory signals.
    if score >= 9.0 and (network_attack or public_exploit_signal):
        likelihood = "CRITICAL"
    elif score >= 7.0:
        likelihood = "HIGH"
    elif score >= 4.0:
    ...
    priority = "P1" if likelihood == "CRITICAL" else "P2" if likelihood == "HIGH" else "P3" if likelihood == "MEDIUM" else "P4"
    confidence = "HIGH" if vector != "N/A" and score > 0 else "LOW"

    remediation = [
        "Identify affected assets and confirm whether vulnerable product versions are present.",
        "Apply the vendor security update or upgrade to a fixed version as soon as change control allows.",
        "If patching is delayed, restrict network access to the affected component and apply vendor workarounds.",
        "Enable or review logging for exploitation attempts against the affected service.",
        f"Review vendor advisories and references: {cve['references'][0] if cve.get('references') else 'N/A'}",
    ]
    ...
    return {
        ...
        "likelihood_justification": (
            f"CVSS base score of {score} with vector {vector}. "
            f"Network attack={network_attack}, no privileges required={no_auth}, no user interaction={no_interaction}, "
            f"high impact={high_impact}, public advisory/exploit signal={public_exploit_signal}."
        ),
        ...
        "limitations": [
            "Rule-based analysis uses CVSS, CWE, reference tags, and description text; it does not confirm exploitability in your environment.",
            "Prioritization should be adjusted using asset exposure, compensating controls, and business criticality.",
        ],
        "note": "Generated by deterministic fallback. Set CLAUDE_API_KEY for optional AI-assisted narrative analysis.",
    }
